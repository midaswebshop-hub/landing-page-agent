...
// Gratis, sin API key. GET completo para forzar generación antes de devolver URL
async function generateWithPollinations(prompt, seed = 0) {
  try {
    ...
    // GET completo para forzar que Pollinations genere la imagen
    // Una vez generada, la URL queda en cache y Shopify puede descargarla
    ...
    if (res.ok) {
      ...
      if (contentType.startsWith("image/")) {
        // Consumir el body para completar la conexión
    ...
    console.error(`[Pollinations] Error (timeout 60s): ${err.message}`);
    return null;
  }
}
