...
import { buildLandingHTML_v7 as buildLandingHTML } from "./landing_html_v7.js";
import {
  createProduct,
  testConnection,
  listProducts,
...
export async function createLandingPage(productInfo) {
  ...
  try {
    ...
    const minImages = uniqueImages.length === 0 ? 4 : 3;
    if (uniqueImages.length < minImages) {
      const needed = Math.min(4, minImages - uniqueImages.length);
      console.log(`[Agent] Pocas imágenes (${uniqueImages.length}) — generando ${needed} con Gemini IA...`);
      try {
        ...
        const geminiImages = await generateProductImages(productInfo.name, {
          ...
          countryCode: productInfo.countryCode || "CO",
        });

        if (geminiImages.length > 0) {
          // Imágenes Gemini van AL INICIO si no hay ninguna otra
        ...
        console.error("[Agent] Error con Gemini imágenes:", err.message);
      }
    }
    ...
    // Auto-detect: si el precio es alto y el país tiene rate > 100, ya es moneda local
    // Ej: sellPrice=8900, country=CR (rate=530) → 8900/530=16.79 USD → es local
    // Ej: sellPrice=29.99, country=CR → 29.99/530=0.056 → es USD
    const priceNum = parseFloat(sellPrice);
    const priceIsLocal = (productInfo.currency && productInfo.currency === countryData.currency)
      || (countryData.rate > 100 && priceNum > countryData.rate * 2);
    let priceDisplay;
    try {
      if (priceIsLocal) {
...
export async function autoCreateFromWebhook(product) {
  ...
  // Determinar país: market del dropshipping → countryCode explícito → default CR
  const countryCode = product.countryCode || product.market || "CR";

  console.log(`[Agent] 🚀 Auto-creando landing PRO para "${product.name}" (score: ${score}, país: ${countryCode})`);

  // Extraer datos de landing pre-generados por el dropshipping agent
  const preGenerated = product.landing_page || {};
  ...
  return createLandingPage({
    ...
    aliExpressUrl: product.aliexpress_url || product.aliExpressUrl || "",
    score: product.score,
    marketingAngle: product.marketing_angle || preGenerated.headline || "",
    preGeneratedCopy: preGenerated,
    autoCreated: true,
    // Pasar datos extra del dropshipping agent
    trendSignal: product.trend_signal || "",
    aliexpressKeywords: product.aliexpress_keywords || "",
    facebookPixelId: process.env.META_PIXEL_ID || null,
  });
}
