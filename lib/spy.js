...
export async function spyCompetitors(productName, options = {}) {
  ...
  const [fbAds, googleResults, shopifyStores, bingResults] = await Promise.all([
    ...
    searchBing(productName).catch(e => { console.log(`[Spy] Bing error: ${e.message}`); return []; }),
  ]);

  results.push(...fbAds, ...googleResults, ...shopifyStores, ...bingResults);
  ...
  console.log(`[Spy] Total: ${unique.length} competidores (FB:${fbAds.length} Google:${googleResults.length} Bing:${bingResults.length} Shopify:${shopifyStores.length})`);
...
async function searchGoogle(query) {
  try {
    ...
    if (!res.ok) { console.log(`[Spy] Google: HTTP ${res.status}`); return []; }
    const html = await res.text();
    const results = [];
    const urlPattern = /href="\/url\?q=(https?:\/\/[^&"]+)/g;
    let match;
    const seen = new Set();
    const blocked = ["google","youtube","wikipedia","facebook","amazon","mercadolibre","instagram","tiktok","twitter","reddit"];

    while ((match = urlPattern.exec(html)) !== null) {
      const url = decodeURIComponent(match[1]);
      try {
        const domain = new URL(url).hostname;
        if (!blocked.some(b => domain.includes(b)) && !seen.has(domain)) {
          seen.add(domain);
...
// ─── BING SEARCH (no bloquea servers) ──────────────────────
async function searchBing(query) {
  try {
    const searchQuery = encodeURIComponent(`${query} comprar tienda online envio`);
    const res = await fetch(`https://www.bing.com/search?q=${searchQuery}&count=10`, {
    ...
    const results = [];
    const seen = new Set();
    const blocked = ["bing","microsoft","google","youtube","wikipedia","facebook","amazon","instagram","tiktok"];

    // Bing results: <a href="https://..." h="ID=SERP,...">
    const urlPattern = /<a[^>]+href="(https?:\/\/[^"]+)"[^>]*>/gi;
    let match;
    while ((match = urlPattern.exec(html)) !== null && results.length < 6) {
      const url = match[1];
      try {
        const domain = new URL(url).hostname;
        if (!blocked.some(b => domain.includes(b)) && !seen.has(domain) && url.length > 20) {
          seen.add(domain);
          // Extraer título cercano
          const afterMatch = html.substring(match.index, match.index + 300);
          const titleMatch = afterMatch.match(/>([^<]{10,100})</);
          results.push({ url, source: "bing", domain, title: titleMatch?.[1] || domain, query });
        }
      } catch {}
    }

    console.log(`[Spy] Bing: ${results.length} tiendas`);
    return results;
  } catch (e) {
    console.log(`[Spy] Bing error: ${e.message}`);
    return [];
  }
}
