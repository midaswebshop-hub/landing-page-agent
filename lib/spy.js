...
export async function spyCompetitors(productName, options = {}) {
  ...
  console.log(`[Spy] Total: ${unique.length} competidores (FB:${fbAds.length} Google:${googleResults.length} DDG:${bingResults.length} Shopify:${shopifyStores.length})`);
...
// ─── DUCKDUCKGO HTML SEARCH (funciona desde servidores) ────
async function searchBing(query) {
  try {
    ...
    const results = [];
    const seen = new Set();
    const blocked = ["duckduckgo","google","youtube","wikipedia","facebook","amazon","mercadolibre","instagram","tiktok","twitter","reddit"];

    // DDG embeds real URLs in uddg= parameter
    const urlPattern = /uddg=([^&"]+)/gi;
    let match;
    while ((match = urlPattern.exec(html)) !== null && results.length < 8) {
      const url = decodeURIComponent(match[1]);
      try {
        const domain = new URL(url).hostname;
        if (!blocked.some(b => domain.includes(b)) && !seen.has(domain) && url.length > 20) {
          seen.add(domain);
          results.push({ url, source: "duckduckgo", domain, query });
        }
      } catch {}
    }

    console.log(`[Spy] DuckDuckGo: ${results.length} tiendas`);
    return results;
  } catch (e) {
    console.log(`[Spy] DuckDuckGo error: ${e.message}`);
    return [];
  }
}
